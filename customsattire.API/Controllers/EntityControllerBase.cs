...
namespace CustomsAttire.API.Controllers
{
    ...
    {
        protected EntityControllerBase(CustomsAttireContext dataContext, IMapper mapper, IValidator<TCreateModel> createValidator, IValidator<TUpdateModel> updateValidator) : base(dataContext, mapper)
        {
        ...
        protected virtual async Task<TReadModel> CreateModel(TCreateModel createModel, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if(identityName == null) { identityName = "Dummy"; }
            createModel.CreatedDate = DateTimeOffset.UtcNow;
            createModel.CreatedBy = identityName;
            createModel.ModifiedDate = DateTimeOffset.UtcNow;
        ...
        protected virtual async Task<TReadModel> UpdateModel(Guid id, TUpdateModel updateModel, CancellationToken cancellationToken = default(CancellationToken))
        {
            var identityName = User?.Identity?.Name;
            if (identityName == null) { identityName = "Dummy"; }
            updateModel.ModifiedDate = DateTimeOffset.UtcNow;
            updateModel.ModifiedBy = identityName;
            if (updateModel.CreatedBy == null)
            {
